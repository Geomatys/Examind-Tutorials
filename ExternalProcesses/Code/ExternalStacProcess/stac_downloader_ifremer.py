...
def process_stac_url(start_url):
    ...
    # --- Asset Selection Logic (Filtered by Role and Alternate Href) ---
    ...
    for key, asset_metadata in assets.items():
        ...
        if "data" in roles:
            ...
            # --- IFREMER Priority Logic ---
            # Check if an alternate HTTPS_IFREMER link exists
            alternates = asset_metadata.get('alternate', {})
            if "HTTPS_IFREMER" in alternates:
                ifremer_href = alternates["HTTPS_IFREMER"].get('href')
                if ifremer_href:
                    print(f"DEBUG: Found IFREMER alternate for asset '{key}'. Prioritizing this link.")
                    selected_asset_href = ifremer_href

            # Fallback to the primary href if no IFREMER alternate was found
            if not selected_asset_href:
                selected_asset_href = asset_metadata.get('href')
                print(f"DEBUG: Using primary href for asset '{key}'.")
    ...
    print(f"Selected asset key: {selected_key}")
